...
static inline void aesctr_seed_r(aesctr_state * state, uint64_t seed) {
    ...
    state->seed[0] = k;
    // D. Lemire manually unrolled following loop since _mm_aeskeygenassist_si128 requires immediates
...
static inline uint32_t aesctr_r(aesctr_state * state) {
    if( __builtin_expect(state->offset >= 16 * AESCTR_UNROLL, 0) ) {
        ...
        for(int i = 0; i < AESCTR_UNROLL; ++i) {
            state->ctr[i] = _mm_add_epi64(state->ctr[i], _mm_set_epi64x(0, AESCTR_UNROLL));

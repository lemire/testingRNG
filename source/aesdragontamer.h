...
static inline void aesdragontamer_seed_r(aesdragontamer_state *state, uint64_t seed) {
  state->increment = _mm_set_epi8(0x2f, 0x2b, 0x29, 0x25, 0x1f, 0x1d, 0x17, 0x13, 
		0x11, 0x0D, 0x0B, 0x07, 0x05, 0x03, 0x02, 0x01);
  state->offset = 0;
  state->state = _mm_set_epi64x(splitmix64_stateless(seed), splitmix64_stateless(seed + 1));
  aesdragontamer_tobuffer_r(state);
}


static inline uint64_t aesdragontamer_r(aesdragontamer_state *state) {
  if(state->offset == 4) aesdragontamer_tobuffer_r(state); // every 4 calls, we generate 4 new uint64_t
...
static inline uint64_t aesdragontamer() { return aesdragontamer_r(&g_aesdragontamer_state); }
